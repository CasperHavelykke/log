...
const dayEntrySchema = z.object({
  ...
  headache: z.boolean(),
  headacheIntensity: z.coerce.number().int().min(1).max(10).nullable(),
  iskiasPain: z.coerce.number().int().min(1).max(5).nullable(),
  alcoholUnits: z.coerce.number().int().min(0).max(50).nullable(),
  constipation: z.boolean(),
  constipationPain: z.coerce.number().int().min(1).max(5).nullable(),
  seborrheicDermatitis: z.coerce.number().int().min(1).max(5).nullable(),
  staph: z.coerce.number().int().min(1).max(5).nullable(),
  ...
  foamyUrinePattern: z.enum(["morning", "all_day"]).nullable(),
...
export async function saveDayEntry(input: DayEntryInput): Promise<SaveResult> {
  ...
  const fields = {
    ...
    headacheIntensity: data.headache ? data.headacheIntensity : null,
    iskiasPain: data.iskiasPain,
    alcoholUnits: data.alcoholUnits,
    constipation: data.constipation,
    constipationPain: data.constipation ? data.constipationPain : null,
    seborrheicDermatitis: data.seborrheicDermatitis,
    staph: data.staph,
    ...
    breathingContext: data.breathingDifficulty
      ...
      : null,
    foamyUrine: data.foamyUrine,
    foamyUrinePattern: data.foamyUrine ? data.foamyUrinePattern : null,
